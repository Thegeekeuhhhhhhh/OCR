...
int train(size_t EPOCHS, double learning_rate)
{
    ...
    for (size_t n = 1; n <= EPOCHS; n++)
    {
        ...
        for (size_t a = 0; a < trainingNumber; a++)
        {
            ...
            training(nn, inputs[trainingIndex[a]], 784, wantedOutputsMatrix);
        } 
        percent = (int)(((double)n / (double)EPOCHS) * 100);
    ...
    for (int r = 0; r < TestNumber; r++)
    {
        ...
        printf("Result : %li, Expected : %li\:qn", ind_test, wantedO[r]);
